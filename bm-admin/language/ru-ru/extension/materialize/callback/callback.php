<?php
// Heading
$_['heading_title']			= 'Обратный звонок';

// Text
$_['text_сalled']			= 'Позвонили';
$_['text_waiting']			= 'Ожидает';
$_['text_success']			= 'Успешно: вы изменили модуль Обратный звонок!';
$_['text_list']				= 'Список заказанных звонков';
$_['text_edit']				= 'Редактирование обратного звонка: #%u';
$_['text_account']			= 'Детальная информация о звонке';
$_['text_info']				= 'Информация';
$_['text_history']			= 'История';
$_['text_history_add']		= 'Добавить историю';
$_['text_ip']				= 'IP';
$_['text_filter']			= 'Фильтры';

// Column
$_['column_id']				= 'ID';
$_['column_telephone']		= 'Телефон';
$_['column_name']			= 'Имя';
$_['column_status']			= 'Статус';
$_['column_date_added']		= 'Дата добавления';
$_['column_comment']		= 'Комментарий';
$_['column_description']	= 'Описание';
$_['column_ip']				= 'IP';
$_['column_action']			= 'Действие';

// Entry
$_['entry_telephone']		= 'Телефон';
$_['entry_name']			= 'Имя';
$_['entry_enquiry']			= 'Комментарий';
$_['entry_call_time']		= 'Время для звонка';
$_['entry_status']			= 'Статус';
$_['entry_comment']			= 'Комментарий';
$_['entry_description']		= 'Описание';
$_['entry_ip']				= 'IP';
$_['entry_order_page']		= 'Страница заказа';
$_['entry_date_added']		= 'Дата добавления';

// Error
$_['error_warning']			= 'Внимание: пожалуйста проверьте форму на наличие ошибок!';
$_['error_permission']		= 'Внимание: у Вас нет прав для редактирования модуля!';
$_['error_telephone']		= 'Телефон должен быть от от 3 до 32 символов!';
$_['error_name']			= 'Имя не может быть длиннее 32 символов!';
$_['error_enquiry']			= 'Комментарий не может быть длиннее 360 символов!';